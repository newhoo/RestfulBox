...
public class MappingLineMarkerProvider implements LineMarkerProvider {
    ...
    public LineMarkerInfo<?> getLineMarkerInfo(@NotNull PsiElement element) {
        if (LanguageHelper.canGenerateLineMarker(element)
            ...
            return new LineMarkerInfo<>(element, element.getTextRange(), ToolkitIcons.REQUEST, Pass.LINE_MARKERS,
                    psiElement -> RestBundle.message("toolkit.navigate.text"),
                    (e, elt) -> RestToolWindowFactory.getRestServiceToolWindow(elt.getProject(), restServiceToolWindow -> {
